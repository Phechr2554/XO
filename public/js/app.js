...
(function () {
  ...
  async function checkAuth() {
    try {
      ...
      authed = true; Store.hydrateRemote(d.profile); applySettings(); UI.setLang(S.settings.lang); renderProfile(); setupNet(); show('home'); maybeJoinDeepLink();
    } catch { authed = false; show('auth'); }
  }

  function setAuthTab(register) {
    $('#tabLogin').classList.toggle('sel', !register); $('#tabRegister').classList.toggle('sel', register);
    ...
    $('#authDesc').textContent = register ? 'สร้างบัญชีเพื่อเก็บชื่อ สถิติ ความสำเร็จ และการตั้งค่าของคุณ' : 'เข้าสู่บัญชีเพื่อเล่นออนไลน์และเก็บข้อมูลของคุณไว้ทุกเครื่อง';
  }

  async function doLogin(e) {
    e.preventDefault();
    try { const d = await api('/api/login', { method:'POST', body: JSON.stringify({ name: $('#loginName').value.trim(), password: $('#loginPassword').value }) }); Store.hydrateRemote(d.profile); authed = true; setupNet(); show('home'); maybeJoinDeepLink(); toast('เข้าสู่ระบบสำเร็จ', 'ok'); }
    catch (err) { toast(err.message, 'error'); }
  }
  async function doRegister(e) {
    e.preventDefault();
    if ($('#registerPassword').value !== $('#registerPassword2').value) return toast('รหัสผ่านยืนยันไม่ตรงกัน', 'error');
    try { const d = await api('/api/register', { method:'POST', body: JSON.stringify({ name: $('#registerName').value.trim(), password: $('#registerPassword').value }) }); Store.hydrateRemote(d.profile); authed = true; setupNet(); show('home'); toast('สมัครสมาชิกสำเร็จ', 'ok'); }
    catch (err) { toast(err.message, 'error'); }
  }
  async function logout() {
    try { Net.socket?.disconnect(); await api('/api/logout', { method: 'POST' }); } catch {}
    authed = false; G.room = null; localStorage.removeItem('xo-arena-v2'); location.href = location.pathname;
  }
  ...
  function bindUI() {
    ...
    $('#tabRegister').onclick = () => setAuthTab(true);
  ...
  function setupNet() {
    ...
    Net.on('auth:required', () => { authed = false; Net.socket?.disconnect(); show('auth'); toast('เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่', 'warn'); });
  ...
  /* ---------------- Deep link ?room=CODE ---------------- */
  function maybeJoinDeepLink() {
    const code = new URLSearchParams(location.search).get('room');
    if (!code || !authed) return;
    const tryJoin = () => { if (!Net.isConnected()) return setTimeout(tryJoin, 250); Net.emit('room:join', { code: code.toUpperCase() }); history.replaceState({}, '', location.pathname); };
    tryJoin();
  }
