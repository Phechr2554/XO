...
(function () {
  ...
  function setupNet() {
    ...
    Net.on('net:error', (err) => { if (err?.message === 'AUTH_REQUIRED') { Store.clearAuth(); Net.disconnect(); goLogin(); } });
  ...
  function goLogin() {
    ...
    location.replace(room ? `/Login?room=${encodeURIComponent(room)}` : '/Login');
  }

  function authResult(result) {
    Store.setAuth({ token: result.token || S.auth.token, userId: result.profile.id });
  ...
  async function initAuth() {
    if (!S.auth.token) return goLogin();
    try {
      const res = await fetch('/api/auth/me', {
        headers: { Authorization: `Bearer ${S.auth.token}`, Accept: 'application/json' },
        credentials: 'same-origin',
        cache: 'no-store'
      });
      if (!res.ok) throw new Error('SESSION');
      authResult({ token: S.auth.token, ...(await res.json()) });
    } catch {
      Store.clearAuth();
      Net.disconnect();
      goLogin();
  ...
  async function logoutAccount() {
    try {
      if (S.auth.token) await fetch('/api/auth/logout', { method: 'POST', headers: { Authorization: `Bearer ${S.auth.token}` }, credentials: 'same-origin', cache: 'no-store' });
    } catch {}
  ...
  function syncAccountData(immediate = false) {
    ...
    const run = async () => {
      ...
      try {
        ...
        else if (r.status === 401) { Store.clearAuth(); Net.disconnect(); goLogin(); }
