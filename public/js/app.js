...
(function () {
  ...
  function bindUI() {
    ...
    $('#btnTheme').onclick = () => { Store.setSetting('theme', S.settings.theme === 'dark' ? 'light' : 'dark'); applySettings(); syncAccountData(); A.play('click'); };
    $('#btnSound').onclick = () => { Store.setSetting('sound', !S.settings.sound); applySettings(); syncAccountData(); A.play('click'); };
    $('#btnLang').onclick = () => { Store.setSetting('lang', S.settings.lang === 'th' ? 'en' : 'th'); applySettings(); UI.setLang(S.settings.lang); syncAccountData(); };
    ...
    $('#btnResetData').onclick = async () => {
      ...
      if (ok) { Store.resetAll(); syncAccountData(true); location.reload(); }
    };
  ...
  function setupNet() {
    ...
    Net.on('welcome', (d) => {
      if (d.profile) { S.profile.id = d.profile.id; S.profile.name = d.profile.name; S.profile.avatar = d.profile.avatar; S.profile.elo = d.profile.elo; Store.applyServerData(d.data); Store.save(); renderProfile(); renderAvatarPicker(); applySettings(); }
    ...
    Net.on('net:up', () => { $('#netDot').className = 'net-dot up'; if (pendingRoomCode) { const code = pendingRoomCode; pendingRoomCode = ''; history.replaceState({}, '', location.pathname); Net.emit('room:join', { code }); } });
    Net.on('net:down', () => { $('#netDot').className = 'net-dot down'; toast('การเชื่อมต่อหลุด กำลังลองใหม่...', 'warn'); });
    Net.on('net:error', (err) => { if (err?.message === 'AUTH_REQUIRED') { Store.clearAuth(); Net.disconnect(); showAuthGate('เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่'); } });
  ...
  function authResult(result) {
    Store.setAuth({ token: result.token, userId: result.profile.id });
    Store.setProfile({ id: result.profile.id, name: result.profile.name, avatar: result.profile.avatar, elo: result.profile.elo });
    Store.applyServerData(result.data);
    renderProfile(); renderAvatarPicker(); applySettings();
    hideAuthGate();
    Net.connect(S.auth.token);
  }
  ...
  function switchAuthMode(register) {
    ...
    $('#authTitle').textContent = register ? 'สมัครสมาชิก' : 'เข้าสู่ระบบ';
    $('#authHint').textContent = register ? 'สร้างบัญชีเพื่อบันทึกข้อมูลและเล่นเกม' : 'ต้องเข้าสู่ระบบก่อนจึงจะเล่นเกมได้';
    $('#authSwitch').textContent = register ? 'มีบัญชีอยู่แล้ว? เข้าสู่ระบบ' : 'ยังไม่มีบัญชี? สมัครสมาชิก';
    $('#authError').textContent = '';
  }
  function initAuth() {
    switchAuthMode(false);
    $('#authSwitch').onclick = () => switchAuthMode(!$('#registerForm').classList.contains('hidden'));
    $('#loginForm').onsubmit = async (e) => {
      e.preventDefault(); $('#authError').textContent = '';
      try { authResult(await authRequest('/api/auth/login', { name: $('#loginName').value, password: $('#loginPassword').value })); $('#loginPassword').value = ''; }
      catch (err) { $('#authError').textContent = err.message; }
    };
    $('#registerForm').onsubmit = async (e) => {
      e.preventDefault(); $('#authError').textContent = '';
      const password = $('#registerPassword').value;
      if (password !== $('#registerPassword2').value) return $('#authError').textContent = 'รหัสผ่านยืนยันไม่ตรงกัน';
      try { authResult(await authRequest('/api/auth/register', { name: $('#registerName').value, password })); $('#registerPassword').value = ''; $('#registerPassword2').value = ''; }
      catch (err) { $('#authError').textContent = err.message; }
    };
    if (S.auth.token) {
      fetch('/api/auth/me', { headers: { Authorization: `Bearer ${S.auth.token}` } })
        .then(async r => { if (!r.ok) throw new Error('SESSION'); return r.json(); })
        .then(d => authResult({ token: S.auth.token, ...d }))
        .catch(() => { Store.clearAuth(); showAuthGate(); });
    } else showAuthGate();
  }
  async function logoutAccount() {
    try { if (S.auth.token) await fetch('/api/auth/logout', { method: 'POST', headers: { Authorization: `Bearer ${S.auth.token}` } }); } catch {}
    if (G.mode === 'online') Net.emit('room:leave');
    Net.disconnect();
    Store.clearAuth();
    S.profile.id = ''; S.profile.name = ''; S.profile.elo = 1000;
    renderProfile(); show('home'); showAuthGate('ออกจากระบบแล้ว');
  }
  let syncTimer = null;
  function syncAccountData(immediate = false) {
    ...
    const run = async () => {
      ...
      syncing = true;
      try {
        const body = Store.exportPlayerData(); delete body.id;
        const r = await fetch('/api/player/data', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${S.auth.token}` }, body: JSON.stringify(body) });
        if (r.ok) { const j = await r.json(); Store.applyServerData(j.data); renderProfile(); }
        else if (r.status === 401) { Store.clearAuth(); Net.disconnect(); showAuthGate('เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่'); }
      } catch {} finally { syncing = false; }
    };
    if (immediate) run(); else syncTimer = setTimeout(run, 200);
  }

  function startQueue() {
    if (!S.auth.token) return showAuthGate('กรุณาเข้าสู่ระบบก่อนเล่น');
  ...
  function renderSettings() {
    ...
    $('#setTheme').onchange = e => { Store.setSetting('theme', e.target.value); applySettings(); syncAccountData(); };
    $('#setStyle').onchange = e => { Store.setSetting('boardStyle', e.target.value); applySettings(); syncAccountData(); };
    $('#setLang').onchange = e => { Store.setSetting('lang', e.target.value); applySettings(); UI.setLang(e.target.value); syncAccountData(); };
    $('#setTimer').onchange = e => { Store.setSetting('localTimer', Number(e.target.value)); syncAccountData(); };
    $('#setSound').onchange = e => { Store.setSetting('sound', e.target.checked); applySettings(); syncAccountData(); };
    $('#setAnim').onchange = e => { Store.setSetting('animations', e.target.checked); applySettings(); syncAccountData(); };
    $('#setVolume').oninput = e => { Store.setSetting('volume', Number(e.target.value)); A.setVolume(Number(e.target.value)); syncAccountData(); };
  }
